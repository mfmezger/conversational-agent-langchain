...
from agent.backend.graph import route_to_retriever, get_chat_history
from agent.backend.nodes.grading import route_to_response_synthesizer
...
def test_route_to_retriever_single_message():
    state = {"messages": [HumanMessage(content="hi")]}
    result = route_to_retriever(state)
    assert result == "retriever"

def test_route_to_retriever_multi_message():
...
def test_route_to_response_synthesizer_default():
...
def test_route_to_response_synthesizer_cohere():
...
def test_retrieve_documents(mock_get_retriever, graph_instance):
    ...
    result = retrieve_documents(state, config)
...
def test_retrieve_documents_with_chat_history(mock_get_retriever, graph_instance):
    ...
    with patch("agent.backend.nodes.retrieval.PromptTemplate.from_template") as mock_prompt, \
         patch("agent.backend.nodes.retrieval.StrOutputParser") as mock_parser:
        ...
        result = retrieve_documents_with_chat_history(state, config, llm=graph_instance.llm, get_chat_history_func=get_chat_history)
...
def test_generate_response(graph_instance):
    ...
    with patch("agent.backend.nodes.generation.ChatPromptTemplate.from_messages") as mock_prompt_cls:
        ...
        result = generate_response_default(state, llm=graph_instance.llm, get_chat_history_func=get_chat_history)
        ...
        result = generate_response_cohere(state, cohere_llm=graph_instance.cohere_llm, llm=graph_instance.llm, get_chat_history_func=get_chat_history)
        assert len(result["messages"]) == 1
